...
@Component({
    selector: 'app-meal-patterns-table',
    templateUrl: './meal-patterns-table.component.html',
    styleUrls: ['./meal-patterns-table.component.scss'],
})
export class MealPatternsTableComponent implements OnInit, OnChanges {
    ...
    drawTable(data) {
        ...
        header
            ...
            .enter()
            .append('th')
            .attr('class', 'flex-head')
            .text((d) => d);

        const tablebody = table.append('tbody').attr('class', 'flex-tbody');
    ...
    updateTable(data) {
        d3.selectAll('.flex-head')
            .data(data.columns)
            .join()
            .text((d) => d);

        const tablebody = d3.select('.flex-tbody');
        const rows = tablebody.selectAll('tr').data(data).join();

        rows.selectAll('.flex-td')
