...
@Component({
    selector: 'app-delivery-methods-table',
    templateUrl: './delivery-methods-table.component.html',
    styleUrls: ['./delivery-methods-table.component.scss'],
})
export class DeliveryMethodsTableComponent implements OnInit, OnChanges {
    ...
    drawTable(data) {
        ...
        const tablebody = table.append('tbody').attr('class', 'delivery-tbody');
    ...
    updateTable(data) {
        ...
        const tablebody = d3.select('.delivery-tbody');
        const rows = tablebody.selectAll('tr').data(data).join();

        rows.selectAll('.delivery-td')
