...
@Component({
    selector: 'app-meal-options-table',
    templateUrl: './meal-options-table.component.html',
    styleUrls: ['./meal-options-table.component.scss'],
})
export class MealOptionsTableComponent implements OnInit, OnChanges {
    ...
    ngOnInit(): void {
        this.http
            .get('../../assets/data/meal-options-full.csv', {
                responseType: 'text',
            })
            .subscribe((data) => {
    ...
    drawTable(data) {
        ...
        const tablebody = table.append('tbody').attr('class', 'options-tbody');
    ...
    updateTable(data) {
        ...
        const tablebody = d3.select('.options-tbody');
        const rows = tablebody.selectAll('tr').data(data).join();

        rows.selectAll('.options-td')
