...
@Component({
    selector: 'app-meal-sites-table',
    templateUrl: './meal-sites-table.component.html',
    styleUrls: ['./meal-sites-table.component.scss'],
})
export class MealSitesTableComponent implements OnInit, OnChanges {
    ...
    drawTable(data) {
        ...
        const tablebody = table.append('tbody').attr('class', 'sites-tbody');
    ...
    updateTable(data) {
        ...
        const tablebody = d3.select('.sites-tbody');
        const rows = tablebody.selectAll('tr').data(data).join();

        rows.selectAll('.sites-td')
