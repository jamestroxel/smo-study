...
@Component({
    selector: 'app-meal-patterns',
    templateUrl: './meal-patterns.component.html',
    styleUrls: ['./meal-patterns.component.scss'],
})
export class MealPatternsComponent implements OnInit, OnChanges {
    ...
    onResize(event: any) {
        ...
        svg.select<SVGGElement>('#flex-x-axis')
            .transition()
        ...
        svg.select<SVGGElement>('#flex-y-axis')
            .transition()
            .ease(d3.easePolyInOut)
            .duration(500)
    ...
    ngOnInit(): void {
        this.http
            .get('../../assets/data/meal-pattern-flexibility-full.csv', {
                responseType: 'text',
            })
            .subscribe((data) => {
    ...
    drawChart(data) {
        ...
        contentGroup
            .selectAll('.flex-filtered')
            .data(data)
            .enter()
            .append('circle')
            .attr('class', 'flex-filtered')
    ...
    updateChart(data) {
        const contentGroup = d3.select('#mealPatterns');
